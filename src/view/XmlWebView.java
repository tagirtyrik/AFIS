    ...
    public String getResponse(){
        ...
        if(airport.size()>0){
            for(int i=0;i<airport.size();i++) {
              //  responce.append("<airport>");
                    responce.append(airport.get(i).toString());//������� ��� ������
               // responce.append("</airport>\n");
            }
        }
        if(plane.size()>0){
            for(int i=0;i<plane.size();i++) {
               // responce.append("<plane>");
                responce.append(plane.get(i).toString());//������� ��� ������
                responce.append("\n");
                //responce.append("</plane>\n");
            }
        }
        if(route.size()>0){
            for(int i=0;i<route.size();i++) {
               // responce.append("<route>");
                responce.append(route.get(i).toString());//������� ��� ������
                responce.append("\n");
               // responce.append("</route>\n");
            }
        }
        if(flight.size()>0){
            for(int i=0;i<flight.size();i++) {
                ...
                responce.append(flight.get(i).toString());//������� ��� ������
                responce.append("\n");
                //responce.append("</flight>\n");
            }
        }
